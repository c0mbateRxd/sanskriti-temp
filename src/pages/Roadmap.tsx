    ...
    <div className="min-h-screen pt-12 pb-20 px-6 max-w-6xl mx-auto space-y-16">
      ...
      <header className="space-y-4">
        ...
        <h1 className="text-7xl font-black tracking-tighter text-slate-900 dark:text-white uppercase leading-none">
          Heritage <br /><span className="text-orange-500 italic">Roadmap</span>
        </h1>
        <p className="text-slate-500 dark:text-white/40 font-medium">
          Track every state you've explored. Unlock milestones as you journey through Bharat.
        </p>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[40px] p-8 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">
                {exploredCount} of {totalStates} states explored
              </p>
            </div>
          ...
          <div className="relative h-6 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden border border-slate-200 dark:border-white/10 p-1">
            <motion.div
              ...
              className="h-full bg-gradient-to-r from-orange-500 via-rose-500 to-indigo-500 rounded-full shadow-lg"
            />
          </div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-rose-600 rounded-[40px] p-8 text-white space-y-4">
          ...
          <div className="text-4xl">
            {unlockedMilestones.length > 0 
              ? unlockedMilestones[unlockedMilestones.length - 1].icon 
              : '🌑'}
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {MILESTONES.map((milestone) => {
            ...
            return (
              <div
                key={milestone.count}
                className={`p-6 rounded-[32px] border text-center space-y-3 transition-all ${
                  unlocked
                    ? 'bg-white dark:bg-white/5 border-orange-500/30 shadow-xl'
                    : 'bg-slate-50 dark:bg-white/2 border-slate-200 dark:border-white/5 opacity-40 grayscale'
                }`}
              >
                <div className="text-4xl">{milestone.icon}</div>
                <p className="text-[9px] font-black uppercase tracking-widest text-slate-900 dark:text-white leading-tight">
                  {milestone.label}
                </p>
                <p className={`text-[8px] font-black uppercase tracking-widest ${unlocked ? 'text-orange-500' : 'text-slate-400'}`}>
                  {milestone.count} States
                </p>
                {unlocked && <CheckCircle2 className="w-4 h-4 text-emerald-500 mx-auto" />}
                {!unlocked && <Lock className="w-4 h-4 text-slate-300 dark:text-white/10 mx-auto" />}
              </div>
            );
          })}
        </div>
