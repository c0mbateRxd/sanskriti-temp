...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 px-6 py-4 ${isScrolled ? 'bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 py-3' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-xl font-black tracking-tighter bg-clip-text text-transparent bg-gradient-to-r from-orange-500 to-rose-600">Sanskriti</span>
        </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                <Link to="/creator-dashboard" className="hidden lg:flex items-center gap-2 px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500 transition-all text-indigo-600 hover:text-white rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-500/20">
              ...
              <button onClick={() => login()} className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all">
        ...
        <button className="md:hidden p-2 rounded-full bg-slate-200 dark:bg-white/5 text-slate-900 dark:text-white" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="absolute top-full left-0 right-0 p-6 bg-white dark:bg-zinc-950 border-b border-slate-200 dark:border-white/10 shadow-2xl md:hidden">
            <div className="flex flex-col gap-6">
              {navLinks.map((link) => (
                <Link key={link.path} to={link.path} onClick={() => setIsMobileMenuOpen(false)} className={`flex items-center gap-4 text-sm font-black uppercase tracking-[0.2em] ${location.pathname === link.path ? 'text-orange-500' : 'text-slate-500 dark:text-white/40'}`}>
